...
public interface CommentReportProcessRepository extends JpaRepository<CommentReportProcess, Long> {
    /**
     * 댓글 신고 상태별 카운트 조회
     */
    @Query(value = """
        SELECT status, COUNT(*) as count 
        FROM comment_report_process_tb 
        GROUP BY status
        """, nativeQuery = true)
    List<Object[]> countByStatus();

    /**
     * 최근 N개월간 월별 댓글 신고 처리 건수
     */
    @Query(value = """
        SELECT 
    ...
    @Query(value = """
        ...
        FROM comment_report_process_tb crp
        WHERE crp.created_at = (
            SELECT MAX(crp2.created_at)
            FROM comment_report_process_tb crp2
